...
public class DoubleTrackBar : Control
{
    ...
    private double RelativeValueLeft
    {
        get
        {
            ...
            return ((diff == 0) ? this.ValueLeft : this.ValueLeft / (double)diff);
        }
    }
    ...
    private double RelativeValueRight
    {
        ...
        {
            var diff = this.Maximum - this.Minimum;
            return ((diff == 0) ? this.ValueLeft : this.ValueRight / (double)diff);
        }
    }
    ...
    {
        var newValue = Math.Min(this.ValueLeft + 1, this.Maximum);
    ...
    {
        var newValue = Math.Min(this.ValueRight + 1, this.Maximum);
    ...
    {
        var newValue = Math.Max(this.ValueLeft - 1, this.Minimum);
        if (this.IsValidValueLeft(newValue))
        {
            this.ValueLeft = newValue;
        }
    ...
    {
        var newValue = Math.Max(this.ValueRight - 1, this.Minimum);
        if (this.IsValidValueRight(newValue))
        {
            this.ValueRight = newValue;
        }
    ...
    private Rectangle GetLeftThumbRectangle(Graphics g = null)
    {
        ...
        var rect = this.GetThumbRectangle(this.RelativeValueLeft, g);
        if (shouldDispose)
        {
            g.Dispose();
        }
    ...
    private Rectangle GetRightThumbRectangle(Graphics g = null)
    {
        ...
        var rect = this.GetThumbRectangle(this.RelativeValueRight, g);
        if (shouldDispose)
        {
            g.Dispose();
        }
    ...
    private Rectangle GetTrackRectangle(int border)
    {
        ...
        return new Rectangle(border, Convert.ToInt32(this.Height / 2.0) - 3, this.Width - 2 * border - 1, 4);
    }

    private Thumbs GetClosestSlider(Point point)
    {
        var leftThumbRect = this.GetLeftThumbRectangle();
        var rightThumbRect = this.GetRightThumbRectangle();
    ...
    protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e)
    {
        ...
        var offset = (this.Maximum - this.Minimum) * e.Location.X;
        offset = (int)(offset / (float)this.Width);
        var newValue = this.Minimum + offset;
        if (draggingLeft)
        {
            if (this.IsValidValueLeft(newValue))
            {
    ...
    protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
    {
        ...
        var thumbSize = this.GetThumbRectangle(0, e.Graphics).Size;
        var trackRect = this.GetTrackRectangle(Convert.ToInt32(thumbSize.Width / 2.0));
    ...
    internal class DoubleTrackBarDesigner : ControlDesigner
    {
        private DoubleTrackBar TrackBar
        {
            get
            {
                return (DoubleTrackBar)this.Control;
            }
        }

        protected override bool GetHitTest(System.Drawing.Point point)
        {
            var pt = this.TrackBar.PointToClient(point);
